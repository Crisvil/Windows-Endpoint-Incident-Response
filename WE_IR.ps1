...
.DESCRIPTION
    Collects key artifacts into a timestamped folder. Highlights potentially suspicious processes.
...
Write-Host "Collecting data to: $outputPath" -ForegroundColor Green

Get-ComputerInfo | Select-Object `
    WindowsProductName, WindowsVersion, OsArchitecture, `
    TotalPhysicalMemory, CsName, CsUserName, CsDomain, `
    BiosBIOSVersion, BiosManufacturer, OsLastBootUpTime |
    Out-File "$outputPath\SystemInfo.txt"

Get-HotFix | Select-Object HotFixID, Description, InstalledBy, InstalledOn |
    Sort-Object InstalledOn -Descending |
    Out-File "$outputPath\Hotfixes.txt"
...
if ($suspicious) {
    ...
    Write-Host "Suspicious processes detected! See Suspicious_Processes.txt" -ForegroundColor Yellow
}
...
Write-Host "Collection complete! Review the folder: $outputPath" -ForegroundColor Green
Write-Host "Tip: Zip the folder and analyze offline." -ForegroundColor Cyan

Write-Host "Exporting Security event log (last 5000 events)..." -ForegroundColor Cyan
try {
    Get-WinEvent -LogName Security -MaxEvents 5000 -ErrorAction Stop |
        Select-Object TimeCreated, Id, LevelDisplayName, ProviderName, Message |
        Export-Csv "$outputPath\SecurityEvents.csv" -NoTypeInformation
    Write-Host "Security events exported successfully." -ForegroundColor Green
} catch {
    "Failed to export Security events: $_" | Out-File "$outputPath\SecurityEvents_Error.txt"
    Write-Host "Failed to export Security events (see SecurityEvents_Error.txt)" -ForegroundColor Red
}
$logs = @("Security", "System", "Application")
foreach ($log in $logs) {
    $file = "$outputPath\$log`Events.csv"
    Write-Host "Exporting $log event log (last 5000)..." -ForegroundColor Cyan
    try {
        ...
        Write-Host "Failed to export $log events: $_" -ForegroundColor Red
    }
}